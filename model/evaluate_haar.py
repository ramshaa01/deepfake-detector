...
------------------------
Day 31: Evaluate the CNN‑only model on the test set using the production 
Haar Cascade face detector (instead of MTCNN) to measure the deployment delta.
"""
...
def main():
    ...
    # Path to the converged CNN‑only checkpoint
    ...
    print("\n--- HAAR CASCADE PRODUCTION PIPELINE RESULTS (CNN‑only) ---")
    ...
    out_path = ROOT / "results" / "day32_detector_delta_v2.md"
    with open(out_path, "w") as f:
        f.write("# Day 32: Production Detector Delta (CNN‑only vs Haar Cascade)\n\n")
        f.write("This evaluates the real‑world deployment accuracy drop caused by using the lightweight Haar Cascade detector with the converged CNN‑only model.\n\n")
        ...
        f.write("| Metric | Day 16 (MTCNN – Official) | Day 32 (Haar Cascade – CNN‑only) | Delta |\n")
        f.write("|---|---|---|---|\n")
