...
To demystify the ~446ms Full API response latency observed on the server, we benchmarked the internal components of the FastAPI endpoint over 10 consecutive requests (CPU). (Note: Earlier measurements of ~256ms or ~73ms referred to 'Model inference only' or partial pipelines, not the complete endpoint execution).
...
| **Full API response latency** | **445.59 ms** | Total internal time (excludes network/FastAPI overhead, but includes all processing). |
